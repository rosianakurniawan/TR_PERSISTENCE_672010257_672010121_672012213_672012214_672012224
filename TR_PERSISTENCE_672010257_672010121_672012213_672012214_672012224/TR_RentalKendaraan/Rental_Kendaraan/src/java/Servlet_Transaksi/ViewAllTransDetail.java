 ...
 * @author RUBY
 */
public class ViewAllTransDetail extends HttpServlet {
EntityManagerFactory emf = Persistence.createEntityManagerFactory("Rental_KendaraanPU");
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (idtr > 0) {
            ...
            RequestDispatcher rd = request.getRequestDispatcher("viewDetailTransAll.jsp");
            rd.forward(request, response);
        } else {
            out.println("<script type=\"text/javascript\">");
            out.println("alert('Gagal View'); window.location.href = 'AllTransSer';");
            out.println("</script>");
        }
