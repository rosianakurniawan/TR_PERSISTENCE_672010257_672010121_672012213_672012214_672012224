...
public class ViewBooking extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        long idbok = Long.valueOf(request.getParameter("idBooking"));
        e_booking bok = bdao.getById(idbok);
        e_Kendaraan ken = bok.getPlatNo();
        e_rental ren = ken.getIdRental();
        if (idbok > 0) {
            request.setAttribute("book", bdao.getById(idbok));
            request.setAttribute("ren", ren);
            request.setAttribute("ken", ken);
            RequestDispatcher rd = request.getRequestDispatcher("viewDetailBooking.jsp");
            rd.forward(request, response);
        } else {
            out.println("<script type=\"text/javascript\">");
            out.println("alert('Gagal View'); window.location.href = 'bookingSer';");
            out.println("</script>");
        }
    }
